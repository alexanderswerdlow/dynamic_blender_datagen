...
def read_channels_from_exr(exr: OpenEXR.InputFile, channel_names: Sequence[str]) -> np.ndarray:
    ...
    for channel_name in channel_names:
        ...
        numpy_type = {Imath.PixelType.HALF: np.float16, Imath.PixelType.FLOAT: np.float32, Imath.PixelType.UINT: np.uint32}[channel_type]
...
def replace_cryptomatte_hashes_by_asset_index(segmentation_ids: None, assets: list):
...
def process_depth(exr_layers, cam_info):
    ...
    def _z_to_depth(z, cam_info):
        ...
        squared_distance_from_center = np.sum(
            np.square(np.meshgrid(pixel_centers_x, pixel_centers_y, indexing="xy")), axis=0  # X-Axis (columns)  # Y-Axis (rows)
        )
...
def process_segementation(exr_layers, scene_info):  # pylint: disable=unused-argument
    ...
    return replace_cryptomatte_hashes_by_asset_index(exr_layers["segmentation_indices"][:, :, :1], scene_info["assets"])
...
def postprocess(from_dir: str, return_layers: Sequence[str], batch_size: int, output_dir: str, frame_idx: int):
    post_processors = {
    ...
    for exr_filename, png_filename in tqdm(zip(exr_frames, png_frames)):
        ...
        for key in return_layers:
            ...
            data = post_processor(source_layers, scene_info)
            if len(data) == 0:
                print(f"{key} is empty for {exr_filename} inside loop")
            data_stack[key].append(data)

        if len(data_stack[return_layers[0]]) == batch_size:
            for key in return_layers:
                print(f"{len(data_stack[key])} for {key} inside loop")

            save_data = {key: np.stack(data_stack[key], axis=0) for key in return_layers}
    ...
    print(f"{len(data_stack[return_layers[0]])} after loop")
    if len(data_stack[return_layers[0]]) > 0:
        save_data = {key: np.stack(data_stack[key], axis=0) for key in return_layers}
...
def write_image_dict(data_dict: Dict[str, np.ndarray], directory: str, file_templates: Dict[str, str] = (), max_write_threads=16, frame_idx=0):
    for key, data in data_dict.items():
        if key in file_templates:
            DEFAULT_WRITERS[key](data, directory, file_template=file_templates[key], max_write_threads=max_write_threads, frame_idx=frame_idx)
        else:
